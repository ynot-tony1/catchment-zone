...
**Most promising next lead for whoever continues after this session:** the address-search-georeferencing technique that unlocked Rotherham (a live webmap's own postcode/address search API for exact ground-control coordinates, rather than visually-estimated landmark pixels) is still worth trying on other Cadcorp-platform councils logged as dead ends in this file's `candidates:` section - **Medway** (`medwaymaps.medway.gov.uk`, dismissed via static-HTML inspection only, "likely OGC endpoint paths all 404... no catchment-specific layer discoverable in the static page HTML" - never actually opened in a live Playwright session the way Rotherham's re-attempt was, so the raster-overlay-with-address-search possibility is genuinely untested there) is the closest match to Rotherham's own situation and the natural next candidate. **Bury** (`map.bury.gov.uk/externalwebmap/`) is a weaker candidate - its note already states community primaries reportedly have no catchments at all, a structural rather than access problem. **North Somerset**'s Cadcorp Aurora server is a different, harder problem (the server itself hangs on `Aurora.svc/Frame` for 45+ seconds from both curl and a live browser) and shouldn't be retried without evidence the server has become responsive. Beyond Cadcorp specifically, Oxfordshire's own remaining ~112 of 307 schools (91 non-gridded landmark-pair PDFs, dwindling `fake_grid_confirmed` backlog) remains the largest bounded body of already-proven-method work in the project.
