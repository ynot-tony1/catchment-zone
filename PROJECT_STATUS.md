...
## Update: Liverpool/Newcastle/Edinburgh "never-touched" claim checked and found false - no new data landed, but Liverpool and Newcastle's dead-end evidence strengthened with explicit council-policy quotes (2026-08-19, later session)

**Baseline at the start of this session: 11,273 `catchment_areas` rows, `map_catchments_cache.feature_count` = 11,273 (in sync), 112 local authorities, HEAD at `edcb967`.** Task brief claimed Liverpool, Newcastle upon Tyne and Edinburgh had "zero entries anywhere in `config/catchment-sources.yml` - not even as declined candidates" and asked for a fresh from-scratch investigation of all three, on the theory this was the same shape of gap as the Crown Dependencies sweep two sessions ago. Checked the premise directly before doing anything else, since the prior session's own closing note explicitly stated the never-touched-LA backlog was "now genuinely empty."

**The premise was wrong for all three, in two different ways.** Edinburgh is not a gap at all: it already has 4 enabled sources (`local_authority_code: "S12000036"`, ND/RC primary+secondary catchment layers from the council's own ArcGIS-backed open-data hub, `last_verified_at: "2026-08-02"`) totalling 123 real catchment polygons, landed in an earlier session alongside Glasgow/Fife/North Lanarkshire's equivalent denominational splits. Liverpool and Newcastle are not undocumented either: both were already named inside an 11-council grouped `candidates:` entry ("Sefton, Liverpool, Knowsley, St Helens, Bolton, Oldham, Rochdale, Sandwell, Walsall, Newcastle upon Tyne, Sunderland") with a real, specific `reason_not_enabled`, and Newcastle specifically had already been individually spot-checked as recently as 2026-08-18 (confirmed the same ArcGIS Experience false-positive pattern found again this session). So this was not a case of a city literally never having been looked at - closer to the audit's own search method (name-matching against `candidates:` entries) missing two cities buried inside one multi-council grouped entry.

**Did the fresh research anyway rather than just trusting the existing notes, and it reproduced and strengthened the same conclusion for both.** Liverpool: the council's own "Who decides where your child goes to school?" admissions page names only individual schools' oversubscription criteria, the last tiebreaker being "live the shortest distance from home to school" - no catchment/priority/designated-area concept appears anywhere on the page or in the linked co-ordinated admission arrangements documents (checked directly, not inferred from a search snippet). Newcastle: the council's own "Legal information - School Admissions" page states outright that straight-line distance from central points is calculated by "a bespoke system that measures land mass including topography," with standards "scrutinised by external agencies, GeoPlace/Ordnance Survey" - an explicit, current, first-party confirmation of pure distance-based admissions. Both cities' web searches keep surfacing the same two generic-titled "ArcGIS Experience" catchment-map apps that have fooled prior sessions' searches for other LAs too (`3f98912c9da44d8c919f50c1525ac813` "School Catchments 2026" and `de7992d0efe34b1c99c319f39addf3c9` "School Catchment Areas") - re-checked their own item metadata directly via `arcgis.com/sharing/rest/content/items/{id}?f=json` and confirmed (again) they are owned by Telford and Wokingham Borough Council respectively, nothing to do with Liverpool or Newcastle. Third-party aggregator sites (catchment.school, locrating, admissionsday) claim catchment maps exist for both cities but cite no primary source and were not treated as verification, per this project's standing rule against inventing or trusting unsourced geometry claims.

**Documentation change: split Liverpool and Newcastle out of the 11-council grouped entry into their own individual `candidates:` entries**, each carrying the specific first-party policy quote found this session, rather than leaving them buried under a generic "no GIS presence" reason shared with 9 other councils that were never re-investigated this session. The remaining 9-council entry (Sefton, Knowsley, St Helens, Bolton, Oldham, Rochdale, Sandwell, Walsall, Sunderland) is otherwise unchanged. No changes were made to Edinburgh's existing 4 enabled sources - they were already correct and did not need re-verification for this task.

**No new catchment data landed this session** - both Liverpool and Newcastle are genuine structural dead ends (pure distance-based admissions, no catchment-zone concept to digitize), and Edinburgh was already fully landed for its ND/RC primary+secondary layers before this session started. `catchment_areas` unchanged at **11,273**; `map_catchments_cache.feature_count` unchanged at 11,273; local authority count unchanged at **112**.

**Verification.** Only `config/catchment-sources.yml`'s `candidates:` section changed (no `sources:` entries added or modified, so `catchment-sources.test.ts`'s `PILOT_LOCAL_AUTHORITIES` fixture needed no update). Confirmed valid YAML via `python3 -c "import yaml; yaml.safe_load(...)"` (82 candidate entries, up from 81 - one entry split into three). `pnpm --filter @catchment-zone/shared sync-config` + `test` (45 passed) and the ingestor's full pytest suite (127 passed, after `uv sync --extra dev` in this fresh worktree) both pass clean. `npx prettier --write` run only against the changed yml file, reported "unchanged" (already correctly formatted). `uv run ingestor verify` passed clean (no local DB available in this worktree, so only non-DB-backed checks ran - expected and consistent with no data having changed). `git status`/`git diff --stat` confirmed only the one intended file changed.

**Lesson for the next never-touched-LA sweep:** name-match candidate entries at the level of individual council names inside multi-council groupings, not just whole-entry titles, before concluding a city was never investigated - this session's false starting premise came from exactly that blind spot. **Genuinely still open for a future session:** Rhondda Cynon Taf's WMS `MAPSOURCE` value (needs live browser network capture); Priority 6 Maps A and C for Hurworth School (Darlington, carried over unchanged); Essex's 14 declined secondary schools (carried over unchanged); Guernsey's secondary layer remains correctly declined as superseded, not open.

## Update: the never-touched-local-authority audit is now definitively, provably closed (2026-08-19, later session)

**No data changed this session** - `catchment_areas` stays at 11,273, `map_catchments_cache.feature_count` stays at 11,273, 112 local authorities. This is a verification-only session prompted by the previous session's false "Liverpool/Newcastle/Edinburgh never touched" premise, which came from an unreliable name-matching audit method. Ran a code-level audit instead, directly against the live database, to settle the question properly rather than risk a third false-premise fork.

**Method: pulled every distinct `(local_authority_code, name)` pair actually present in the live `schools` table's `local_authorities` lookup (242 rows) and diffed it against every `local_authority_code` in `config/catchment-sources.yml`'s `sources:` list (112 codes) plus every name mentioned in the `candidates:` list (120 distinct names, including names bundled inside multi-council grouped entries).** This is a strictly more reliable method than the prior session's name-only search: it uses the DB's own code as the join key wherever possible, so naming variants (e.g. "Guernsey Offshore Establishments" in the DB vs "Guernsey (Channel Islands)" in the registry) can't produce a false "never touched" result the way they did last session.

**Raw diff initially showed 11 apparent gaps; all 11 were confirmed false positives on inspection, not real gaps:**

- `702`/`704`/`708`/`701` "BFPO/Fieldwork/Gibraltar/Scotland Offshore Establishments" and `000` "Does not apply" - these are GIAS placeholder codes for overseas military bases, fieldwork centres, and records with no real local authority, not real UK local authorities. Correctly out of scope.
- `666` "Powys" - a genuine false positive worth noting: Powys **is** already fully covered (2 enabled sources, `primary_catchment` and `secondary_catchment`, landed in an earlier session), but the registry stores it under `local_authority_code: "W-666"` (a `W-` prefix convention used for several Welsh authorities, e.g. Rhondda Cynon Taf's `W-674`, Neath Port Talbot's `W-671`) while the live `schools` table stores the same authority as bare `"666"`. This prefix mismatch is worth a maintenance note: any future automated code-based audit must strip a leading `W-` before comparing, or it will keep flagging every already-covered Welsh authority as a false gap.
- `835`/`836`/`837` "Pre-LGR 2019 Dorset/Poole/Bournemouth", `928` "Pre-LGR 2021 Northamptonshire", `909` "Pre-LGR 2023 Cumbria" - these are superseded pre-reorganisation codes retained on older `schools` rows for historical continuity. Confirmed each successor authority is already accounted for under its current code/name: Dorset and Bournemouth, Christchurch and Poole are both enabled; North Northamptonshire is enabled and West Northamptonshire is a documented candidate; Cumberland is enabled and Westmorland and Furness is a documented candidate. No real gap.

**Conclusion: every one of the 242 local authorities the live database actually contains schools for is either enabled (112) or has an explicit, evidenced `candidates:` entry explaining why it isn't (120, some entries covering multiple councils).** The never-touched-LA angle that produced real wins for Jersey, Isle of Man, and Guernsey two sessions ago is now conclusively exhausted - there is no such thing as an entirely unlooked-at UK local authority left in this project. Any future session tempted to re-run a "never touched" sweep should treat this as settled and spend the effort elsewhere instead: re-attempting already-declined candidates with new techniques (e.g. Rhondda Cynon Taf's live WMS endpoint, which still needs a real browser network capture to discover its `MAPSOURCE` parameter - `google-chrome` is installed in this environment but no CDP/Playwright automation library is, so this needs `playwright install chromium` or a raw `--remote-debugging-port` CDP session set up first), or hunting for a second real document for LAs already flagged `_partial`.

**Verification.** Query: `SELECT code, name FROM local_authorities ORDER BY name` (242 rows) diffed in Python against a `yaml.safe_load` of `config/catchment-sources.yml`'s `sources[].local_authority_code` set and `candidates[].local_authority_name` (comma-split for grouped entries). No files changed other than this entry.

**Local authority count unchanged at 112.** **Most promising next lead:** Rhondda Cynon Taf's WMS `MAPSOURCE` discovery via a real browser network capture - the one candidate blocker in the whole registry that is confirmed blocked by tooling availability alone, not by a real data/access limitation.
