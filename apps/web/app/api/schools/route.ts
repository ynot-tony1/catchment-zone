...
export async function GET(request: NextRequest) {
  ...
  let filters;
  try {
    filters = parseSchoolSearchParams(Object.fromEntries(request.nextUrl.searchParams));
  } catch (error) {
  ...
  try {
    ...
    return jsonResponse(result, { requestId, cacheControl: "public, max-age=60, stale-while-revalidate=300" });
  } catch (error) {
