...
export async function GET(request: NextRequest) {
  ...
  let query;
  try {
    query = parseMapSchoolsQuery(Object.fromEntries(request.nextUrl.searchParams));
  } catch (error) {
  ...
  try {
    ...
    const schools = await prisma.school.findMany({
      where: {
        status: query.status && query.status.length > 0 ? { in: query.status } : "OPEN",
      ...
      select: { urn: true, schoolName: true, phaseName: true, status: true, latitude: true, longitude: true },
      take: query.limit,
    });
    ...
    const featureCollection = {
      ...
      features: schools.map((school) => ({
        ...
        geometry: { type: "Point" as const, coordinates: [school.longitude, school.latitude] },
        properties: {
          urn: school.urn,
    ...
    return jsonResponse(featureCollection, { requestId, cacheControl: "public, max-age=60, stale-while-revalidate=300" });
  } catch (error) {
