...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimit.allowed) {
    const res = errorResponse("RATE_LIMITED", "Too many catchment checks. Please wait before trying again.", requestId);
  ...
  if (!parsed.success) {
    return errorResponse(
      "BAD_REQUEST",
      parsed.error instanceof z.ZodError ? parsed.error.issues.map((issue) => issue.message).join(" ") : "Invalid request.",
      requestId,
    );
  }
  ...
  try {
    ...
    if (!outcome.ok) {
      return errorResponse("NOT_FOUND", "That postcode could not be found. Check it and try again.", requestId);
    }
    return jsonResponse(outcome.result, { requestId });
  } catch (error) {
    return internalErrorResponse(requestId, "POST /api/admissions/check", error);
  }
}
