...
import { parseLocalAuthoritySearchParams } from "@schoolscope/shared";
import { z } from "zod";
...
export async function GET(request: NextRequest) {
  ...
  let filters;
  try {
    filters = parseLocalAuthoritySearchParams(Object.fromEntries(request.nextUrl.searchParams));
  } catch (error) {
  ...
  try {
    ...
    return internalErrorResponse(requestId, "GET /api/local-authorities", error);
  }
}
