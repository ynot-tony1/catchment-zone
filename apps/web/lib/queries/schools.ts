...
function toItem(
  row: SearchRow,
  fromPoint: { lat: number; lon: number } | null,
...
function buildWhere(filters: SchoolSearchFilters): Prisma.SchoolWhereInput {
  ...
  if (filters.q)
    where.normalisedName = { contains: filters.q.trim().toLowerCase() };
  if (filters.postcode)
  ...
  if (filters.establishmentTypeCode)
    where.establishmentTypeCode = filters.establishmentTypeCode;
  if (filters.trustId) where.trustId = filters.trustId;
  if (filters.gender) where.gender = filters.gender;
  if (filters.religiousCharacter)
    where.religiousCharacter = filters.religiousCharacter;
 ...
 */
async function searchByKeyset(
  filters: SchoolSearchFilters,
): Promise<SchoolSearchResult> {
  const prisma = getPrismaClient();
  const where = buildWhere(filters);
  const descending =
    filters.sort === "name_desc" || filters.sort === "opening_date_desc";
  const byOpeningDate =
    filters.sort === "opening_date_asc" || filters.sort === "opening_date_desc";

  const sortField: "normalisedName" | "openingDate" = byOpeningDate
    ? "openingDate"
    : "normalisedName";
  const cursor = filters.cursor
    ? decodeCursor<{ k: string | number | null; u: string }>(filters.cursor)
    : null;
  ...
  if (cursor) {
    ...
    const keyCondition: Prisma.SchoolWhereInput =
      cursor.k === null
        ? {
            [sortField]: descending ? { not: null } : null,
            urn: { [comparator]: cursor.u },
          }
        : {
            OR: [
  ...
  const nextCursor =
    hasMore && last
      ? encodeCursor({
          k: byOpeningDate
            ? (last.openingDate?.toISOString() ?? null)
            : last.normalisedName,
          u: last.urn,
        })
      : null;
...
async function searchByDistance(
  ...
  const items = rows
    ...
    .filter(
      (item): item is SchoolSearchResultItem & { distanceKm: number } =>
        item.distanceKm !== null && item.distanceKm <= radiusKm,
    )
    .sort((a, b) => a.distanceKm - b.distanceKm || a.urn.localeCompare(b.urn));

  const cursor = filters.cursor
    ? decodeCursor<{ k: number; u: string }>(filters.cursor)
    : null;
  const startIndex = cursor
    ? items.findIndex(
        (item) =>
          item.distanceKm > cursor.k ||
          (item.distanceKm === cursor.k && item.urn > cursor.u),
      )
    : 0;
  const page = items.slice(
    Math.max(startIndex, 0),
    Math.max(startIndex, 0) + filters.limit,
  );
  const hasMore = Math.max(startIndex, 0) + filters.limit < items.length;
  const last = page[page.length - 1];
  const nextCursor =
    hasMore && last ? encodeCursor({ k: last.distanceKm, u: last.urn }) : null;
